...
Module PTree <: TREE.
  ...
  (* This is CompCert's original [set], which shadows when key is already in the map *)
  ...
  (* added: 
     this version of set takes a value [def], and a function [f] that
  ...
  (* This version of set only adds the element if the key wasn't already present
     "set ONLY if Fresh" *)
  Fixpoint set_only_f (A : Type) (i : positive) (v : A) (m : t A) {struct i} : t A :=
    match m with
    | Leaf =>
        match i with
        | xH => Node Leaf (Some v) Leaf
        | xO ii => Node (set_only_f ii v Leaf) None Leaf
        | xI ii => Node Leaf None (set_only_f ii v Leaf)
        end
    | Node l o r as t =>
        match i with
        | xH => t
        | xO ii => Node (set_only_f ii v l) o r
        | xI ii => Node l o (set_only_f ii v r)
        end
    end.
  ...
  - assert (REMOVE: xelements (remove i (Node m1 o m2)) j nil =
    ...
    + destruct (IHm1 i (xO j) H) as (l1 & l2 & EQ & EQ').
      ...
      split.
      rewrite xelements_node, EQ. rewrite <- app_assoc. auto.
      rewrite xelements_node, EQ', ! app_assoc. auto.
...
Module PMap <: MAP.
  ...
  Definition set_only_f (A : Type) (i : positive) (v : A) (m : t A) :=
    (fst m, PTree.set_only_f i v (snd m)).
